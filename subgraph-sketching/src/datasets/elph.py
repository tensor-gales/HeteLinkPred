...
import torch_sparse
from torch_geometric.nn.conv.gcn_conv import gcn_norm, Adj, OptTensor, Optional, \
    SparseTensor, is_torch_sparse_tensor, maybe_num_nodes, add_remaining_self_loops, to_edge_index, \
        scatter, add_self_loops_fn, set_sparse_value
...
def sage_norm(  # noqa: F811
    ...
    if isinstance(edge_index, SparseTensor):
        raise NotImplementedError("Sparse Tensors are not yet supported in 'sage_norm'")
    ...
    deg_inv_sqrt.masked_fill_(deg_inv_sqrt == float('inf'), 0)
    edge_weight = edge_weight * deg_inv_sqrt[row]
...
class HashDataset(Dataset):
    ...
    def __init__(
            ...
            directed=False, grid = False, **kwargs):
        ...
        if self.directed:  # make directed graphs like citation2 undirected
            print(
    ...
    def _generate_sign_features(self, data, edge_index, edge_weight, sign_k):
        ...
        if self.sign_norm == "gcn":
            edge_index, edge_weight = gcn_norm(  # yapf: disable
                edge_index, edge_weight.float(), num_nodes)
        elif self.sign_norm == "sage":
            edge_index, edge_weight = sage_norm(  # yapf: disable
                edge_index, edge_weight.float(), num_nodes)
            # test = torch_sparse.spmm(edge_index, edge_weight, data.x.shape[0], data.x.shape[0], data.x)
            # t0 = test[50, :]
            # agg_test = data.x[edge_index[0, edge_index[1, :] == 50]].mean(axis=0)
            # assert torch.allclose(t0, agg_test, atol=1e-5), 'sage norm is not working'
        else:
            raise ValueError(f"sign_norm {self.sign_norm} not recognized")
        if sign_k == 0:
            if self.sign_norm == 'gcn':
    ...
    def _preprocess_node_features(self, data, edge_index, edge_weight, sign_k=0):
        ...
        """
        _, year_str, _ = self._generate_file_names(1)
        if self.sign_norm == 'gcn':
            if sign_k == 0:
                feature_name = f'{self.root}_{self.split}{year_str}_featurecache.pt'
            else:
                feature_name = f'{self.root}_{self.split}{year_str}_k{sign_k}_featurecache.pt'
        else:
            if sign_k == 0:
                feature_name = f'{self.root}_{self.split}{year_str}_{self.sign_norm}_featurecache.pt'
            else:
                feature_name = f'{self.root}_{self.split}{year_str}_{self.sign_norm}_k{sign_k}_featurecache.pt'
    ...
    def _generate_file_names(self, num_negs):
        ...
        elif self.args.dataset_name == 'grid':
            year_str = f'idx_{self.args.graph_idx}'
        else:
            year_str = ''
...
def get_grid_hash_datasets(dataset, train_data, val_data, test_data, args, directed=False, dataset_name="grid"):
    root = f"./src/datasets/{dataset_name}_subgraph_features/{dataset_name}_hash"
    ...
    train_dataset = HashDataset(root, 'train', train_data, pos_train_edge, neg_train_edge, args,
                                use_coalesce=False, directed=directed, grid=True)
