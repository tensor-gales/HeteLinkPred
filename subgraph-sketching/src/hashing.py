...
class ElphHashes(object):
    ...
    def _get_intersections(self, edge_list, hash_table):
        ...
        for k1 in range(1, self.max_hops + 1):
            for k2 in range(1, self.max_hops + 1):
                src_hll = hash_table[k1]['hll'][edge_list[:, 0].to(torch.long)]
                src_minhash = hash_table[k1]['minhash'][edge_list[:, 0].to(torch.long)]
                dst_hll = hash_table[k2]['hll'][edge_list[:, 1].to(torch.long)]
                dst_minhash = hash_table[k2]['minhash'][edge_list[:, 1].to(torch.long)]
    ...
    def get_subgraph_features(self, links, hash_table, cards, batch_size=11000000):
        ...
        for batch in tqdm(link_loader):
            ...
            cards1, cards2 = cards.to(links.device)[links[batch, 0].to(torch.long)], cards.to(links.device)[links[batch, 1].to(torch.long)]
