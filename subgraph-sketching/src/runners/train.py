...
def get_train_func(args):
    if args.model == 'ELPH':
        ...
        train_func = train_gprgnn
    else:
        train_func = train
    return train_func


def train_buddy(model, optimizer, train_loader, args, device, logger, emb=None):
...
def train_gprgnn(model, optimizer, train_loader, args, device, logger):
    """
    train a GNN that calculates hashes using message passing
    @param model:
    @param optimizer:
    ...
    for batch_count, indices in enumerate(tqdm(loader)):
        ...
        print(indices.size())
        # logits = model.predictor(subgraph_features, batch_node_features, batch_emb)
        loss = torch.nn.functional.nll_loss(out[indices].to(device), labels[indices].squeeze(0).to(device))
