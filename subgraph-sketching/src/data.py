...
from torch_geometric.utils import (add_self_loops, negative_sampling,
                                   to_undirected)
from src.negative_sampling import vector_to_edge_index, edge_index_to_vector, sample
...
from src.datasets.elph import get_hashed_train_val_test_datasets, make_train_eval_data, get_grid_hash_datasets
import dgl
...
def get_grid_loaders_BUDDY(args, dataset, device, splits, dataset_root, directed=False):

    print("Getting Dataloader")
    
    train_data, val_data, test_data = splits['train'], splits['valid'], splits['test']

    print("Getting hash datasets")
    train_dataset, val_dataset, test_dataset = get_grid_hash_datasets(dataset, train_data, val_data,
                                                                                      test_data, args, directed)
    print("Got hashed datasets")
...
def get_grid_splits(args, data, split_edges):
    splits = {}
    print(split_edges.keys())
    for key in split_edges.keys():
        if key != "train":
            neg_edges = torch.stack((split_edges[key]["source_node"].reshape(-1, 1).repeat(1,split_edges[key]["target_node_neg"].shape[1]).reshape(-1),split_edges[key]["target_node_neg"].reshape(-1))).swapaxes(0,1)
            pos_edges = torch.stack([split_edges[key]["source_node"], split_edges[key]["target_node"]]).swapaxes(0,1)
            ...
            pos_edges = torch.stack([split_edges[key]["source_node"], split_edges[key]["target_node"]])
            neg_edges = negative_sampling(
                    data.edge_index, num_nodes=data.num_nodes,
                    num_neg_samples=pos_edges.size(1)* args.num_negs)
            n_pos, n_neg = pos_edges.shape[1], neg_edges.shape[1]
            edge_label = torch.cat([torch.ones(n_pos), torch.zeros(n_neg)], dim=0)
            print(neg_edges.shape)
            print(pos_edges.shape)
        ...
        splits[key] = Data(x=data.feat, edge_index=edge_index, edge_weight=edge_weight, edge_label=edge_label,
                        edge_label_index=edge_label_index)
    return splits
    
def get_grid_data(args, device, dataset_root):
    print("Get Grid Data")
    graph_idx = args.graph_idx

    base_filename = dataset_root + "/dense_new-" + str(graph_idx)
    ...
    split_edges = torch.load(base_filename + "-train-split.pt")
    
    dataset = from_dgl(g)
    splits = get_grid_splits(args,dataset, split_edges)

    return dataset, splits, False, 'mrr'


def get_esci_loaders_BUDDY(args, dataset, device, splits, dataset_root, directed=False):
    print("Getting Dataloader")
...
def get_esci_data(args, device, dataset_root, val_num_negs=1000):
    print("Get ESCI Data")
    ...
    load_split['train_pos'] = load_split['train_pos'].swapaxes(0,1)
    
    if hasattr(dataset, "edge_weight"):
            edge_weight = dataset.edge_weight
    else:
        edge_weight = torch.ones(dataset.edge_index.shape[1])


    neg_train_edges = negative_sampling(
            dataset.edge_index, num_nodes=dataset.num_nodes,
            num_neg_samples=load_split['train_pos'].size(1)* args.num_negs)

    edge_label = torch.cat([torch.ones(load_split['train_pos'].shape[1]), torch.zeros(neg_train_edges.shape[1])], dim=0)
    edge_label_index = torch.cat([load_split['train_pos'], neg_train_edges], dim=1)
    
    edge_split["train"] = Data(x = dataset.feats, edge_index = dataset.edge_index, edge_weight = edge_weight
                               , edge_label = edge_label, edge_label_index = edge_label_index)
    
    load_split['valid_pos'] = load_split['valid_pos'].swapaxes(0,1)
    neg_valid_edges = neg_valid_edges.swapaxes(0,1)

    neg_valid_edges = torch.stack((load_split['valid_pos'][0, :].reshape(-1, 1).repeat(1,neg_valid_edges.shape[0]).reshape(-1),neg_valid_edges.reshape(-1)))


    edge_label = torch.cat([torch.ones(load_split['valid_pos'].shape[1]), torch.zeros(neg_valid_edges.shape[1])], dim=0)
    edge_label_index = torch.cat([load_split['valid_pos'] , neg_valid_edges], dim=1)

    edge_split["valid"] = Data(x = dataset.feats, edge_index = dataset.edge_index, edge_weight = edge_weight
                               , edge_label = edge_label, edge_label_index = edge_label_index)
    
    load_split['test_pos'] = load_split['test_pos'].swapaxes(0,1)
    neg_test_edges = neg_test_edges.swapaxes(0,1)
    neg_test_edges = torch.stack((load_split['test_pos'][0,:].reshape(-1, 1).repeat(1,neg_test_edges.shape[0]).reshape(-1),neg_test_edges.reshape(-1)))


    edge_label = torch.cat([torch.ones(load_split['test_pos'].shape[1]), torch.zeros(neg_test_edges.shape[1])], dim=0)
    edge_label_index = torch.cat([load_split['test_pos'], neg_test_edges], dim=1)

    edge_split["test"] = Data(x = dataset.feats, edge_index = dataset.edge_index, edge_weight = edge_weight
                               , edge_label = edge_label, edge_label_index = edge_label_index)


    return dataset, edge_split, False, 'mrr'



def get_loaders(args, dataset, splits, directed):
    print("Get Grid Data Loaders")
    train_data, val_data, test_data = splits['train'], splits['valid'], splits['test']
    ...
    test_loader = dl(test_dataset, batch_size=args.batch_size, shuffle=shuffle_test,
                     num_workers=args.num_workers)
    if (args.dataset_name == 'ogbl-citation2') and (args.model in {'ELPH', 'BUDDY'}) and False:
